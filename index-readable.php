...
$methods = array(
	'roll' => function($sender, $arg) {
		...
		if (is_numeric($arg) && $arg > 0 && $arg <= 6) {
			...
			$rolls = ", guess what happens when you roll negative dice... you get a 0. Congratulations, you lose";
		} else if ($arg > 6) {
			$rolls = ", I only have 6 dice D:";
		}
	...
	'weather' => function() {
		$xml = simplexml_load_file('http://weather.yahooapis.com/forecastrss?w=' . GEO_CODE);
		$current = $xml->channel->item->description;
		$start = strpos($current, 'Current Conditions:') + strlen('Current Conditions:</b><br />');
		$end = strpos($current, '<BR />', $start);
		echo trim(str_replace(' F', '&#176; F', substr($current, $start, $end - $start)));
	}
);
